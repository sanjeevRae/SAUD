...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  if (!isAllowed) {
    return (
<main className="grid min-h-screen place-items-center bg-[#f4f1ed] px-5 text-[#111111]">        <div className="mx-auto max-w-xl text-center">
<p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#8f1f35]">404</p>          <h1 className="mt-4 text-4xl font-semibold">Permission denied</h1>
      ...
      <section className="bg-[#111111] px-5 pb-16 pt-8 text-white">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#d6b1bb]">Saud Leather</p>
...
<article key={label} className="border border-white/10 bg-white/[0.06] p-5 backdrop-blur">                <div className="flex items-start justify-between gap-3">
                  <div>
 <p className="text-xs uppercase tracking-[0.16em] text-white/45">{label}</p>                    <p className="mt-2 text-lg font-semibold">{helper}</p>
                  </div>
