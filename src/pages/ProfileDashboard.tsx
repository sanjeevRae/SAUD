...
export default function ProfileDashboard() {
  ...
  useEffect(() => {
    ...
    const loadOrdersAndReviews = async () => {
      ...
      const [ordersResponse, reviewsResponse] = await Promise.all([
        fetch(`/api/customer/orders?userId=${encodeURIComponent(user.id)}`),
        fetch(`/api/customer/reviews?userId=${encodeURIComponent(user.id)}`),
      ]);
  ...
  const deleteReview = async (item: OrderItem) => {
    ...
    const response = await fetch(`/api/customer/reviews?userId=${encodeURIComponent(user.id)}&productId=${encodeURIComponent(item.id)}`, { method: 'DELETE' });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
